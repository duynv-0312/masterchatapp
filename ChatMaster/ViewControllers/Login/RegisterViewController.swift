...
class RegisterViewController: UIViewController {
    ...
    @objc
    private func didTapChangeProfile() {
    ...
    private func loginButtonTapped() {
        ...
        DatabaseManager.shared.userExits(with: email, completion: { [weak self] exits in
            guard let strongSelf = self else {
                return
            }
            guard !exits else {
                strongSelf.alertUserLoginError(message: "Look like an user account for that email address already exits")
                return
            }
